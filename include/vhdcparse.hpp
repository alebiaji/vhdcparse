 ...
 * --------------------------
 * The parser uses a lexer generated by 'flex' (see sdc_lexer.l), and a parser
 * generated by 'bison' (see sdc_parser.y).
 *
 ...
 *          a) Add the new token definition to sdc_parser.y (e.g. ARG_HOLD)
 *          b) Add a pattern to sdc_lexer.l which returns the token (e.g. '-hold')
 *          c) Add a new (optional) rule to the appropriate command in sdc_parser.y
 ...
 *          a) Add the new token definition to sdc_parser.y (e.g. CMD_SET_TIME_FORMAT)
 *          b) Add a pattern to sdc_lexer.l which returns the token (e.g. 'set_time_format')
 *          c) Add a new rule for the command to sdc_parser.y e.g.:
