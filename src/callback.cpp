...
void PrintCallback::create_clock(const CreateClock& cmd) {
...
void PrintCallback::set_io_delay(const SetIoDelay& cmd) {
    printf("#%s:%d\n", filename_.c_str(), lineno_);
...
void PrintCallback::set_clock_groups(const SetClockGroups& cmd) {
    printf("#%s:%d\n", filename_.c_str(), lineno_);
...
void PrintCallback::set_false_path(const SetFalsePath& cmd) {
    printf("#%s:%d\n", filename_.c_str(), lineno_);
...
void PrintCallback::set_min_max_delay(const SetMinMaxDelay& cmd) {
    printf("#%s:%d\n", filename_.c_str(), lineno_);
...
void PrintCallback::set_multicycle_path(const SetMulticyclePath& cmd) {
    printf("#%s:%d\n", filename_.c_str(), lineno_);
...
void PrintCallback::set_clock_uncertainty(const SetClockUncertainty& cmd) {
    printf("#%s:%d\n", filename_.c_str(), lineno_);
...
void PrintCallback::set_clock_latency(const SetClockLatency& cmd) {
    printf("#%s:%d\n", filename_.c_str(), lineno_);
...
void PrintCallback::set_disable_timing(const SetDisableTiming& cmd) {
    printf("#%s:%d\n", filename_.c_str(), lineno_);
...
void PrintCallback::set_timing_derate(const SetTimingDerate& cmd) {
    printf("#%s:%d\n", filename_.c_str(), lineno_);
...
void PrintCallback::current_design(const CurrentDesign& cmd) {
    printf("#%s:%d\n", filename_.c_str(), lineno_);
    printf("current_design ");
    if (!cmd.empty) {
        std::cout << cmd.current_design.strings.back() << std::endl;
    }
}
